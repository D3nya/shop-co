...
import { NAVIGATION_ITEMS } from "./model/navConstants";
  ...
  return (
    <>
      ...
      <div className={classNames(className, "xl:hidden relative flex items-center")} ref={ref}>
        ...
        >
          <NavigationLinks className="flex flex-col items-center space-y-4 py-2" onClick={toggleBurger} navItems={NAVIGATION_ITEMS} />
        </nav>
      </div>

      {/* Desktop */}
      <nav className={classNames(className, "hidden xl:block")}>
        <NavigationLinks className="flex gap-6" navItems={NAVIGATION_ITEMS} />
      </nav>
    </>
  );
};
